...
export class App extends Component {
    ...
    if (this.state.input.length === 0) {
      ...
      try {
        let result = await axios.get(
          `https://api.openweathermap.org/data/2.5/weather?q=${this.state.input}&appid=${process.env.REACT_APP_API_KEY}&units=imperial`
        );
  ...
  render() {
    ...
    return (
      <div className="app" style={{ textAlign: "center", marginTop: "8%" }}>
        ...
        <input
          ...
          name="input"
        />
        <button className="submit" style={{ textAlign: "center", marginTop: "3%" }} onClick={this.handleSearchSubmit}>Submit</button>
